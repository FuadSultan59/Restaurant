...
function CreateReception() {
    ...
    function HandleSignup (event) {
        ...
        fetch('http://localhost:3232/reception/Create-Reception', {
            ...
            if(response.ok) 
            {
                response.json().then((data) =>{
                    ...
                    if (data.message === 'email_exist') {
                        ...
                        setParagraph('Reception Succesfully Created with Email Address: ')
                        setModal(true)      
                    }
                    else if(data.message === 'invalid_token') {
                        setHeader('Invalid Token')
                        setParagraph(' Please Login Again! Failed Regestering Reception with Email: ')
                        setModal(true)                 
                    }
                    else {
                        setinfo("Error!")
    ...
    <main className="Recption-container">
        <section className="left">
        <h1 style={{color: 'darkgreen'}}> Register<span style={{color: 'darkgreen',fontWeight: 'bolder'}}>-Reception</span></h1>
        ...
        <form className='recep-signup-form' type='submit' >
            ...
            <button className="button-green" onClick={HandleSignup}>Register</button>
